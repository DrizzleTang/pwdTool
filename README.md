...
| 3 | 展示框交互 | 以鼠标位置为左上角展示；点击框外任意位置自动收起；上下键选择、回车确认 |
| 4 | 浏览器导入 | 读取 Chrome / Edge 已保存的密码（旧版加密可直读，新版需配合 CSV 导入，见下文说明） |
| 5 | 托盘图标 + 设置页 | 系统托盘常驻，双击/右键打开设置 |
| 6 | 设置项 | 展示框大小、背景色/高亮色、是否"主动填写"、开机自启动、全局快捷键 |
| 7 | 自动填写 | 选中账号后模拟输入"账号 + Tab + 密码"到原窗口；失败时自动改为复制密码到剪贴板 |
| + | 标签分类 | 每条账号可打多个标签（如"工作"、"银行"），搜索框同时匹配标签 |
| + | 主密码（可选） | 默认仅用 Windows DPAPI 加密账号库，设置里可开启"主密码"二次加密 |

## 直接下载使用（无需安装 .NET）

`publish/PwdTool-win-x64-v1.0.0.zip` 是**自包含单文件发布**（已内置完整 .NET 8 运行时），
在 Windows 上解压后双击 `PwdTool.exe` 即可运行，无需另外安装任何运行时。
...
（开始菜单快捷方式、卸载条目、可选开机自启）。本项目在当前开发所用的 Linux 沙箱环境里
交叉编译好了可执行程序，但沙箱自带的 NSIS 3.09 版本在编译"包含安装向导页面 + 我们这个
体积的自包含单文件"时会稳定触发其自身的内部崩溃（与文件路径、文件名、压缩器种类都无关，
定位后判断是这份 NSIS 二进制自身的问题，非本项目代码问题）。因此本次交付以**解压即用的
zip 包**为准；如果你有一台正常的 Windows 机器（或其它未受此问题影响的 NSIS 环境），
可以直接用以下命令生成正式的 `PwdTool-Setup-1.0.0.exe` 一键安装程序：
...
5. 设置窗口分四个页签：
   - **账号管理**：新增/编辑/删除账号（含标签），从 Chrome/Edge 导入，或从 CSV 导入；
   - **展示框外观**：宽高、背景色、选中高亮色；
   - **常规**：是否主动填写、开机自启、全局快捷键（点击快捷键框后直接按下新组合）；
   - **安全**：开启/关闭/修改主密码。
...
- 默认仅用 **Windows DPAPI**（`CurrentUser` 范围）加密账号库文件
  ...
  局限：同一账户下运行的其它程序理论上也能用 DPAPI 解密该文件。
- 设置里可选开启"**主密码**"：在 DPAPI 之外，再用 PBKDF2-SHA256（迭代 ≥ 210,000）
  派生密钥、AES-256-GCM 加密账号库。开启后每次启动需要输入主密码，安全性更高，
  但**丢失主密码将无法恢复账号库**，请务必牢记。
- `settings.json`（展示框大小/颜色、是否自动填写、开机自启、快捷键等）为明文存储，
  不包含任何密码。
...
src/
  ...
  Program.cs              入口：单实例 Mutex + 启动托盘上下文
  TrayAppContext.cs        托盘常驻上下文：图标/菜单/热键/弹窗生命周期
  Models/AccountEntry.cs   账号模型（含标签、排序权重）
  Services/
    AppSettings.cs         非敏感设置的 JSON 持久化
    PasswordStore.cs       账号库 CRUD + DPAPI/主密码两层加密
    HotkeyManager.cs       全局热键 (RegisterHotKey) 封装
    AutoTypeService.cs     SendInput 自动填写
    AutoStartManager.cs    开机自启（注册表 Run 项）
    BrowserImporter.cs     Chrome/Edge 直读 + CSV 导入
    Native.cs              集中的 P/Invoke 声明
  Forms/
    PopupForm.cs           鼠标位置弹窗（搜索 + 列表）
    SettingsForm.cs        设置窗口（四个页签）
    EditEntryForm.cs        新增/编辑账号（含标签）
    MasterPasswordForm.cs   主密码 解锁/设置/修改 对话框
installer/
  PwdTool.nsi              NSIS 一键安装脚本
publish/
...
由于开发环境是 Linux 沙箱、无法运行 WinForms 界面，请在 Windows 上按以下清单
逐项手动验证：

- [ ] 双击 `PwdTool.exe`，任务栏出现托盘图标，无多余窗口
- [ ] 按 `Ctrl+Alt+P`，弹窗出现在鼠标位置左上角，搜索框自动获得焦点
- [ ] 输入关键字，列表按标题/账号/网址/标签实时过滤
- [ ] 点击弹窗外部，弹窗自动消失；再次按快捷键，弹窗重新出现（toggle）
- [ ] 选中一条账号回车，账号+密码被填写到之前聚焦的输入框（或复制到剪贴板+气泡提示）
- [ ] 设置 → 账号管理：新增/编辑/删除账号，标签能被搜索框搜到
- [ ] 设置 → 展示框外观：调整宽高/颜色后弹窗立即生效
- [ ] 设置 → 常规：录制新快捷键、切换主动填写、切换开机自启（重启后仍生效）
- [ ] 设置 → 安全：开启主密码后重启程序需要输入主密码；忘记密码场景已在 UI 提示
- [ ] 设置 → 账号管理：从 Chrome/Edge 导入，能看到导入结果与"新版密码已跳过"的数量提示
- [ ] 卸载/删除后 `%AppData%\PwdTool\` 下的 `vault.dat`/`settings.json` 清理正常
